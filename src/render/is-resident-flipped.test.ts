...
test("右向きのたい焼きは鏡像反転する（魚形）", () => {
  expect(isResidentFlipped({ species: "taiyaki", dir: 1 })).toBe(true);
});

test("左向きのたい焼きは反転しない（原画の向き）", () => {
  expect(isResidentFlipped({ species: "taiyaki", dir: -1 })).toBe(false);
});
