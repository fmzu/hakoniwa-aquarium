...
/** 指定した値を順に返し、尽きたら throw する乱数（想定外の消費を検出する） */
function sequenceRandom(values: number[]): () => number {
  let i = 0;
  return () => {
    if (i >= values.length) {
      throw new Error(`乱数の消費が想定 ${values.length} 回を超えた`);
    }
    return values[i++];
  };
}
...
test("発見済みの種が空なら throw（呼び出し側の契約違反）", () => {
  expect(() => createVisitor([], 100, 300000, sequenceRandom([0.5]))).toThrow(
    "発見済みの種が0件",
  );
});
